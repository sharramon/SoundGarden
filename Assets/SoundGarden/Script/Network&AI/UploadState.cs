...
public class UploadState : Singleton<UploadState>
{
    CheckResult_Request_Protocol _Protocol;

    // Hand 
    public GameObject[] handStateObjects;
    public HandState _HandState;
    private string filePath;
    public Transform seedInitPos;   // 생성될 씨앗 위치, 손에 포지션 위치 
    public SeedCollider handCollider;
    
    // Send Data
    private int selectedVoiceModelId;
    ...
    void Send2AIserver(string wavFilePath)
    {
        ...
        audio_Request_Protocol.VoiceModelId = selectedVoiceModelId; //TODO 악기 선택
        ...
        NetworkManager.Instance.Send(_Protocol, (CheckResult_Response_Protocol _Response) =>
            {
                if (_Response.outputFileUrl != null)
                {
                    ...
                    StartCoroutine(DownloadFile(_Response.outputFileUrl, StringTool.ConnectString(Application.persistentDataPath, NetworkManager.Instance.networkSetting.SavePath,
    ...
    private IEnumerator PlayAudioFromFile(string filePath)
    {
        using (UnityWebRequest www = UnityWebRequestMultimedia.GetAudioClip("file://" + filePath, AudioType.WAV))
        {
            ...
            else
            {
                ...
                AudioSource source = SoundManager.Instance.CheckAudioSource(_Seed);
                source.clip = clip;
                source.loop = false;
